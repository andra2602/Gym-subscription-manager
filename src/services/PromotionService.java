...
public class PromotionService {
    ...
    public List<Promotion> getPromotions() {
        return promotionDAO.readAll();
    }

    public void checkPromotionValidityOnStartup() {
        List<Promotion> allPromotions = promotionDAO.readAll();

        for (Promotion p : allPromotions) {
            if (p.getEndDate().isBefore(LocalDate.now()) && p.isActive()) {
                p.setActive(false);
    ...
    public void editPromotion(Scanner scanner) {
        List<Promotion> activePromotions = promotionDAO.readAll().stream()
                .filter(Promotion::isActive)
                .toList();
    ...
    public void removePromotion(Scanner scanner) {
        ...
        System.out.print("Enter the number of the promotion to remove: ");
        int index = scanner.nextInt();
        scanner.nextLine();

        if (index < 1 || index > allPromotions.size()) {
            System.out.println("❌ Invalid selection.");
            return;
        }

        Promotion selected = allPromotions.get(index - 1);

        // Delete din baza de date
    ...
    public void deactivatePromotion(Scanner scanner) {
        ...
        Promotion selected = activePromos.get(index - 1);
        selected.setActive(false);
        boolean updated = promotionDAO.update(selected);
